...
# ----- Core Task Functions -----
def add_task_bulk(tasks, daily_tasks):
    ...
    for entry in task_entries:
        ...
        start_time_24h = start_hour * 60 + start_minute  # Convert to total minutes for comparison
        ...
        if is_recurring:
            ...
            print("\nDEBUG: CURRENT DAILY TASKS ->", daily_tasks)  # 🔍 See how tasks are stored
        else:
...
def view_tasks(tasks, daily_tasks):
    ...
    for task in daily_tasks:
        category = task.get("category")  # ✅ Pull from stored category instead of asking again
        if category and category in tasks:
            ...
            categorized_recurring_tasks["Not Urgent but Important"].append(task)  # Default category fallback
    ...
    for category, task_list in tasks.items():
        ...
        for i, task in enumerate(task_list, 1):
            ...
            print(f"   📅 Date: {task['date']} | ⏳ Duration: {duration} | ⏰ Start: {start_time}")

        # Display recurring tasks in the same category
        for i, task in enumerate(categorized_recurring_tasks[category], len(task_list) + 1):
            description = task.get("description", "No description")
            start_time = task.get("start_time", "TBD")  # ✅ Fix: Use "start_time" instead of "time"
            print(f"   {i}. {description} | ⏰ {start_time} (RECURRING)")
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            add_task_bulk(tasks, daily_tasks)
        elif choice == "2":
            view_tasks(tasks, daily_tasks)
        elif choice == "3":
            remove_task(tasks)
        elif choice == "4":
