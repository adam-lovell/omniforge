...
def add_burdens(tasks, daily_tasks):
    ...
    for entry in task_entries:
        ...
        for task in tasks[category]:
            existing_hour, existing_minute = map(int, task['start_time'].split(":"))
            existing_am_pm = task['start_time'].split(" ")[1]
...
def shed_burdens(tasks, daily_tasks):
    """Remove a selected task or recurring burden."""
    if not tasks and not daily_tasks:
        print("\n⚠️ NO BURDENS TO REMOVE.")
        return

    # ✅ Ensure `view_burdens()` is correctly referenced
    try:
        view_burdens(tasks, daily_tasks)  # ✅ Show current burdens before removal
    except NameError:
        print("\n⚠️ view_burdens() FUNCTION MISSING. ENSURE IT IS DEFINED.")
        return
    
    print("\nENTER THE CATEGORY OF THE BURDEN TO REMOVE.")
    category = input("\nCATEGORY: ").strip()

    if category in tasks and tasks[category]:
        for i, task in enumerate(tasks[category], 1):
            print(f"{i}: {task['description']} | ⏰ {task['start_time']} | 📅 {task['date']}")
        
        try:
            task_num = int(input("\nENTER BURDEN NUMERAL TO REMOVE: ").strip())
            if 1 <= task_num <= len(tasks[category]):
                removed = tasks[category].pop(task_num - 1)
                print(f"\n✅ BURDEN LIFTED: '{removed['description']}'")
            else:
                print("\n⚠️ INVALID SELECTION.")
        except ValueError:
            print("\n⚠️ INVALID INPUT.")

    elif category.lower() == "recurring":
        for i, task in enumerate(daily_tasks, 1):
            print(f"{i}: {task['description']} | ⏰ {task.get('time', 'TBD')} (RECURRING)")

        try:
            task_num = int(input("\nENTER NUMERAL TO REMOVE RECURRING BURDEN: ").strip())
            if 1 <= task_num <= len(daily_tasks):
                removed = daily_tasks.pop(task_num - 1)
                print(f"\n✅ RECURRING BURDEN LIFTED: '{removed['description']}'")
            else:
                print("\n⚠️ INVALID SELECTION.")
        except ValueError:
            print("\n⚠️ INVALID INPUT.")
    ...
    # ✅ Save updated burdens after changes
...
def progress_report(tasks, daily_tasks):
    ...
    for task in daily_tasks:
        task_time = task.get("time", "TBD")  # ✅ Fix: Default missing 'time' to "TBD"
